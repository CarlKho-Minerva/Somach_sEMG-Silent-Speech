...
% \usepackage{enumitem}  % not available in basic texlive
\usepackage{url}
% \usepackage{natbib}  % using thebibliography instead
% \usepackage{textcomp}  % included in modern LaTeX kernels
...
\hypersetup{
    ...
    pdftitle={Curriculum Learning for Silent Speech Classification},
...
Section~\ref{sec:hardware} describes hardware. Section~\ref{sec:signal} covers signal processing and feature extraction. Section~\ref{sec:curriculum} presents the curriculum learning protocol. Section~\ref{sec:architecture} details the classification architecture. Section~\ref{sec:results} reports results. Section~\ref{sec:discussion} discusses implications and limitations. Section~\ref{sec:conclusion} concludes.
...
\textbf{Total Cost:} \$40 (including breadboard, jumper wires, 50-pack electrodes).
...
\textbf{Technical Nuance: Mel-Scaling Linearity.} At our 250\,Hz sampling rate (125\,Hz Nyquist), the Mel-scale is essentially linear. The extraction effectively functions as a linear filterbank analysis, but it preserves the critical \emph{amplitude-invariance} property identified by prior work~\cite{kapur2018}, which is essential for cross-intensity generalization.
...
\textbf{Our approach:} Train on a \emph{curriculum} of five conditions spanning the full speech-intensity range, including a critical intermediate phase that bridges the gap between audible and silent speech.

\subsection{The Five-Phase Spectrum}

\begin{table}[h]
\centering
\caption{Five-phase curriculum design.}
\label{tab:curriculum}
\begin{tabular}{@{}lllll@{}}
\toprule
Phase & Condition & Mouth & Amplitude ($\mu$V) & Instruction \\
\midrule
1 & Overt         & Open     & 50--150 & Speak aloud normally \\
2 & Whispered     & Open     & 30--80  & Whisper without vocal cords \\
3 & Mouthing      & Open     & 20--50  & Move lips silently (lip sync) \\
5 & Exaggerated   & \textbf{Closed} & 10--30  & Clench jaw, ``shout internally'' \\
6 & Covert        & \textbf{Closed} & 2--10   & Think the word; minimize all movement \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering
\caption{Six-class directional vocabulary.}
\label{tab:vocab}
\begin{tabular}{@{}ll@{}}
\toprule
Class & Articulatory Description \\
\midrule
UP      & Tongue-to-palate, jaw elevation \\
DOWN    & Tongue root depression \\
LEFT    & Lateral tongue pressure \\
RIGHT   & Tongue tip curl \\
SILENCE & Jaw relaxed, rest state \\
NOISE   & Swallowing, head turning, non-speech movements \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{LSTM failure is diagnostic.} The LSTM achieves exactly 16.6\% (chance level for 6 classes), indicating that it predicts the same class for all inputs. LSTMs model \emph{sequential dependencies} across time steps, but the AD8232 captures only the 80\,ms neuromuscular onset burst---there is no temporal structure beyond this spike for recurrent gates to exploit. The LSTM's failure is therefore \emph{confirmatory evidence} that the system operates as an onset detector, not a continuous speech decoder.
...
\textbf{Note:} The curve below shows training accuracy only. No validation or test split was used; see~\S\ref{sec:limitations} for discussion.
...
\textbf{Honest framing:} The 48.9\% held-out accuracy should not be directly compared to AlterEgo's 92\%, as the hardware cost differs by 25$\times$ and the evaluation protocols differ. Meltzner et al.'s 83\% was achieved with laryngectomy patients who produce exaggerated articulatory signals, not healthy-subject covert speech. The appropriate comparison is with Nieto et al.'s inner speech EEG (\textasciitilde40\% held-out on 4 classes using \$5,000 hardware), where our \$40 system achieves competitive performance (48.9\% on 6 classes). The multi-session result (58.2\%) demonstrates that with proper electrode-shift data collection, consumer hardware enters the range of practical utility.
...
Thanks to Prof.\ Watson for advising this work and the ``Palm Pilot insight''---that teaching the user a consistent motor strategy (specific tongue positions per command) reduces variance the model must learn. Thanks to Prof.\ Choi (IL181 Computational Neuroscience, Minerva University) for foundational EMG/neuroscience knowledge. This work was partially supported by the Minerva University capstone program (CP193/CP194).
...
\bibitem{hristov2026}
D.\ Hristov, T.\ Ganev, and K.\ Ivanov, ``Time2Vec transformer for robust gesture recognition from low-density sEMG,'' \emph{arXiv preprint}, arXiv:2602.01855, 2026.

\bibitem{kho2026companion}
C.~V.~L.\ Kho, ``Lingual-mandibular electrode configuration for silent speech: Throat sensor necessity on consumer-grade ADCs,'' \emph{arXiv preprint} (companion paper, submitted concurrently), 2026.

\bibitem{gaddy2020}
D.\ Gaddy and D.\ Klein, ``Digital voicing of silent speech,'' in \emph{Proc.\ EMNLP}, 2020, pp.\ 5521--5530.

\bibitem{biosense2016}
N.\ Merrill, T.\ Chuang, and J.\ Chuang, ``Passthoughts authentication with low cost earEEG,'' in \emph{Proc.\ EMBC}, 2016.
